...
- `MessageBubble.tsx`: Animated user and assistant message card. Reads `message.provider` directly to show the correct provider logo per bubble (logos can differ across messages in the same chat). Renders a collapsible `<details>`-style block for `isContextHandoff` messages. Shows a spinning provider logo while the assistant message is streaming but empty.
- `ProviderSwitchDialog.tsx`: Modal warning shown when the user selects a different provider on a session that already has messages. Presents "Yes, switch" / "Cancel" options. Always shown — there is no suppress option until an app-settings screen exists.
