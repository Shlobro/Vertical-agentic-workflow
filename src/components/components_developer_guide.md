...
- `MessageBubble.tsx`: Animated user and assistant message card, including typing placeholder behavior. Accepts `highlightQuery` to render inline `<mark>` highlights, and `isHighlightTarget` to add a yellow ring around the targeted message bubble.
