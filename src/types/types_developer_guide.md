...
## Current Scope
- `index.ts`: Provider union, message/session/project interfaces, Tauri event payloads, provider catalog, and model catalog used by the UI. The OpenAI/Codex catalog currently exposes GPT-5.4 and GPT-5.3 Codex entries with reasoning-effort variants encoded as `model:effort`.
