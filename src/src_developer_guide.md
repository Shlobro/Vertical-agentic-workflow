...
4. Choosing `New project` opens the folder picker first. After the user selects a folder, `App.tsx` asks Tauri which of `CLAUDE.md`, `AGENTS.md`, and `GEMINI.md` are missing in that project root. If any are missing, the shell opens an in-app checklist dialog, preselecting the user's remembered defaults, and can create the selected files before continuing. That dialog also supports editing one shared template for all three files, restoring the built-in system template, and optionally remembering a custom template for future new projects. If the chosen folder already contains `.Vertical` state, `App.tsx` loads it through `load_project_state`; otherwise the store creates a project named after that folder plus one empty chat inside it.
...
7. A debounced save path invokes `save_workspace_state` after relevant project/chat changes, sidebar-ratio changes, companion-file default-selection changes, and remembered companion-template changes so project folders and the workspace registry stay synchronized with the in-memory store.
